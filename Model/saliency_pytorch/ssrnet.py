...
class SSRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        bu1 = F.interpolate(bu1, size=block2.shape[2:], mode="bilinear")
        bu2 = self.refinement2(block2, bu1)
        bu2 = F.interpolate(bu2, size=block1.shape[2:], mode="bilinear")
        bu3 = self.refinement3(block1, bu2)
        bu3 = F.interpolate(bu3, size=x.shape[2:], mode="bilinear")
        seg = self.decoder(bu3)
        return seg

    def freeze_bn(self):
        for m in self.resnet.named_modules():
                if isinstance(m[1], nn.BatchNorm2d):
                    m[1].eval()

    def freeze_layer(self):
        for layer in [self.resnet.conv1, self.resnet.bn1, self.resnet.layer1, self.resnet.layer2]:
